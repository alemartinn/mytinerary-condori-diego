...
export default function EditCity() {
    ...
    const [cityToEdit, setIdCityToEdit] = useState("")
    const formOdel = [
        {name: 'city', type: 'text'},
    ...
    const viewForm = (input, i) => (
        ...
        <div className='form__group field' key={i}>
            <InputMod clase='form__field' id={input.name} type={input.type} name={input.name} min={`${input.type === 'number' && input.min}`} max={`${input.type === 'number' && input.max}`}></InputMod>
    ...
    const editingCity = async (e) => {
        ...
        .then(response => setIdCityToEdit(response.data.response))
        .catch(error => console.log(error))
    }
